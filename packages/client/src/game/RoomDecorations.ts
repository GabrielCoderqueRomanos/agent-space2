...
export class RoomDecorations {
  ...
  private decorateHangar(scene: Phaser.Scene) {
    ...
    // ── DOCKED FIGHTER SHIP (center-right, prominent) ─────────────────
    const fsx = cx - S * 0.3, fsy = cy - S * 1.2;

    // Landing struts (6, triangular)
    g.fillStyle(P.metal);
    for (const [sx, sy, dx] of [
      [fsx - 28, fsy + 50, -8], [fsx + 28, fsy + 50, 8],
      [fsx - 20, fsy + 70, -5], [fsx + 20, fsy + 70, 5],
    ] as [number, number, number][]) {
      g.fillTriangle(sx, sy, sx + dx, sy + 18, sx - dx, sy + 18);
      g.lineStyle(1, P.metalHlt, 0.5);
      g.strokeTriangle(sx, sy, sx + dx, sy + 18, sx - dx, sy + 18);
    }

    // Fuselage (main body)
    g.fillStyle(P.metalMid);
    g.fillRect(fsx - 18, fsy, 36, 72);

    // Wing panels (swept back)
    g.fillStyle(P.metal);
    g.fillTriangle(fsx - 18, fsy + 20, fsx - 46, fsy + 60, fsx - 18, fsy + 60);
    g.fillTriangle(fsx + 18, fsy + 20, fsx + 46, fsy + 60, fsx + 18, fsy + 60);

    // Wing edge highlights
    g.lineStyle(1, P.metalHlt, 0.5);
    g.lineBetween(fsx - 18, fsy + 20, fsx - 46, fsy + 60);
    g.lineBetween(fsx + 18, fsy + 20, fsx + 46, fsy + 60);

    // Nose section (tapered)
    g.fillStyle(P.metalBrt);
    g.fillTriangle(fsx, fsy - 14, fsx - 14, fsy + 4, fsx + 14, fsy + 4);
    g.fillStyle(P.metalMid);
    g.fillRect(fsx - 14, fsy + 4, 28, 16);  // nose body

    // Cockpit dome
    g.fillStyle(P.glassDim);
    g.fillEllipse(fsx, fsy + 16, 20, 14);
    g.lineStyle(1, P.glass, 0.7);
    g.strokeEllipse(fsx, fsy + 16, 20, 14);
    // Canopy glare
    g.fillStyle(P.glass, 0.35);
    g.fillEllipse(fsx - 3, fsy + 13, 8, 5);

    // Engine nacelle bumps (rear of wings)
    for (const nx of [fsx - 34, fsx + 34]) {
      g.fillStyle(P.metal);
      g.fillEllipse(nx, fsy + 52, 12, 18);
      g.lineStyle(1, P.metalHlt, 0.4);
      g.strokeEllipse(nx, fsy + 52, 12, 18);
      // Exhaust glow
      g2.fillStyle(0x3355aa, 0.3);
      g2.fillEllipse(nx, fsy + 62, 8, 6);
    }

    // Hull panel lines
    g.lineStyle(1, P.metalHlt, 0.3);
    g.lineBetween(fsx - 18, fsy + 36, fsx + 18, fsy + 36);
    g.lineBetween(fsx - 18, fsy + 52, fsx + 18, fsy + 52);
    g.lineBetween(fsx, fsy + 4, fsx, fsy + 68);

    // Nav light (green right, red left)
    g2.fillStyle(0x00ff44, 0.9);
    g2.fillCircle(fsx + 44, fsy + 56, 2.5);
    g2.fillStyle(0xff2200, 0.9);
    g2.fillCircle(fsx - 44, fsy + 56, 2.5);
